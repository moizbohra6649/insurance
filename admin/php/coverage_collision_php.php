...
$coverage_collision_id = (isset($_REQUEST["coverage_collision_id"])) ? $_REQUEST["coverage_collision_id"] : 0;
...
if(isListInPageName(pathinfo($_SERVER['PHP_SELF'], PATHINFO_FILENAME))){
    $select_query = "SELECT * FROM coverage_collision";
...
switch ($mode) {
    ...
    case "INSERT":
        ...
        $select_qry = mysqli_query($conn, "SELECT id FROM coverage_collision WHERE minimum_amount = '$_REQUEST[minimum_amount]' and maximum_amount = '$_REQUEST[maximum_amount]' " );
        ...
        if(mysqli_num_rows($select_qry) > 0){
            $error_arr[] = "This Coverage Collision is already exists.<br/>";
        }
        ...
        $insert_query = mysqli_query($conn, "INSERT INTO coverage_collision (coverage_collision_id, prefix_coverage_collision_id, minimum_amount, maximum_amount, status) VALUES ('$coverage_collision_id', '$prefix_coverage_collision_id', '$minimum_amount', '$maximum_amount', 1) ");
        ...
        if (!mysqli_commit($conn)) {
            ...
            $data["msg"] = "Coverage Collision inserted successfully.";
            $data["status"] = "success";
        } else {
    ...
    case "EDIT":
        ...
        $select_query = mysqli_query($conn, "SELECT * FROM coverage_collision where id = '$id' ");
        
        if(mysqli_num_rows($select_query) > 0){
            $get_data = mysqli_fetch_array($select_query);

            $coverage_collision_id            = $get_data["coverage_collision_id"];
            $prefix_coverage_collision_id     = $get_data["prefix_coverage_collision_id"];
    ...
    case "UPDATE":
        ...
        $select_qry_data = mysqli_query($conn, "SELECT * FROM coverage_collision WHERE id = '$id' " );
        $select_qry = mysqli_query($conn, "SELECT id FROM coverage_collision WHERE minimum_amount = '$_REQUEST[minimum_amount]' and maximum_amount = '$_REQUEST[maximum_amount]' AND id != '$id'" );
        ...
        $get_coverage_collision_id = $get_qry["id"]; 
            
        // Turn autocommit off
        mysqli_autocommit($conn,FALSE);
            
        $update_qry = mysqli_query($conn, "UPDATE coverage_collision SET minimum_amount = '$minimum_amount',maximum_amount = '$maximum_amount', updated = now() WHERE id = $id");
        ...
        if (!mysqli_commit($conn)) {
            ...
            $data["msg"] = "Coverage Collision updated successfully.";
            $data["status"] = "success";
        } else {
            $data["msg"] = "Query error please try again later.";
            $data["status"] = "error";
