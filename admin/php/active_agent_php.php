...
$message = '' ;
if($activation_id  > 0 ){
    $select_agent = mysqli_query($conn, "SELECT * FROM agent WHERE id = $activation_id" );
    if(mysqli_num_rows($select_agent) > 0){
        $get_user = mysqli_fetch_assoc($select_agent);
        if($get_user['status'] == 1){
            $message = "The agent account is already approved and ready to use.";
        }else{
            mysqli_autocommit($conn,FALSE);
            $update_query = mysqli_query($conn, "UPDATE agent SET status = 1 , updated = now() WHERE id = $activation_id");
            if (!mysqli_commit($conn)) {
                $message = 'Something went wroung.' ;
            }else if (!empty($update_query)) {
                $message = 'Agent has been successfully activated. The account is now active and ready to use.' ;
            } else {
