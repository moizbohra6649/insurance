...
class FPDF {
    ...
    */
    ...
    function Row($data, $border       = 0, $fill         = 0, $ht           = 5) {
        //Calculate the height of the row
        $nb           = 0;
        ...
        for ($i = 0;$i < count($data);$i++) {
            ...
            //Print the text
            // $this->MultiCell($w, 5, $data[$i], 0, $a);
            // $this->MultiCellTag($w, $ht, $data[$i], 0, $a, $fill, 0, 0, 0, 0);
            $this->MultiCell($w, 5, $data[$i], 0, $a);
            //	 $this->Row($data[$i],$border,$fill,$ht);
            //Put the position to the right of the cell
            $this->SetXY($x + $w, $y);
        }
        //Go to the next line
    ...
    function CellFit($w, $h          = 0, $txt        = '', $border     = 0, $ln         = 0, $align      = '', $fill       = 0, $link       = '', $scale      = 0, $force      = 1) {
        ...
        if ($fit) $this->_out('BT ' . ($scale == 0 ? '0 Tc' : '100 Tz') . ' ET');
    }

    function CellFitScale($w, $h        = 0, $txt      = '', $border   = 0, $ln       = 0, $align    = '', $fill     = 0, $link     = '') {
        //***************************************************
        //neeru 26 apr 13 copy condition from Cell() bcoz some times footer (page no) is not print on page
    ...
    protected function _httpencode($param, $value, $isUTF8) {
        ...
        if (!$isUTF8) $value = $this->_UTF8encode($value);
        return $param . "*=UTF-8''" . rawurlencode($value);
    }

    protected function _UTF8encode($s) {
        // Convert ISO-8859-1 to UTF-8
        if ($this->iconv) return iconv('ISO-8859-1', 'UTF-8', $s);
        $res = '';
        $nb  = strlen($s);
        for ($i   = 0;$i < $nb;$i++) {
            $c = $s[$i];
    ...
    protected function _UTF8toUTF16($s) {
        ...
        $res = "\xFE\xFF";
        if ($this->iconv) return $res . iconv('UTF-8', 'UTF-16BE', $s);
        $nb = strlen($s);
        $i  = 0;
        while ($i < $nb) {
            $c1 = ord($s[$i++]);
    ...
    protected function _out($s) {
        ...
        elseif ($this->state == 1) $this->Error('Invalid call');
    ...
    protected function _putpages() {
        ...
        for ($i  = 1;$i <= $nb;$i++) {
            $this->PageInfo[$i]['n'] = ++$n;
            $n++;
            foreach ($this->PageLinks[$i] as & $pl) $pl[5] = ++$n;
            unset($pl);
        }
    ...
    function OpenTag($tag, $attr, $hw  = 5) {
        //Opening tag
        switch ($tag) {
            case 'STRONG':
                $this->SetStyle_H('B', true);
            break;
            case 'EM':
                $this->SetStyle_H('I', true);
            break;
            case 'B':
            case 'I':
            case 'U':
                $this->SetStyle_H($tag, true);
            break;
            case 'A':
                $this->HREF = $attr['HREF'];
            break;
        ...
        function CloseTag($tag) {
            ...
            if ($tag == 'STRONG') $tag = 'B';
            if ($tag == 'EM') $tag = 'I';
            if ($tag == 'B' || $tag == 'I' || $tag == 'U') $this->SetStyle_H($tag, false);
            if ($tag == 'A') $this->HREF = '';
            if ($tag == 'FONT') {
        ...
        function SetStyle_H($tag, $enable) {
        ...
        function PutLink($URL, $txt, $hw = 5) {
            //Put a hyperlink
            $this->SetTextColor(0, 0, 255);
            $this->SetStyle_H('U', true);
            $this->Write($hw, $txt, $URL, 'J');
