...
$usermessage = (isset($_REQUEST["usermessage"])) ? $_REQUEST["usermessage"] : '';
 ...
 if($form_request == "true"){
    ...
    mysqli_autocommit($conn,FALSE);
     ...
     $insert_query = mysqli_query($conn, "INSERT INTO contact (username, email, usermessage) VALUES ('$username', '$email', '$usermessage') ");
        ...
        if (!mysqli_commit($conn)) {
            ...
            $body = file_get_contents(dirname(__DIR__) . '/partial/contact_welcome_email_template.php');
            $body = str_replace('{{name}}', htmlspecialchars($username), $body);
            send_mail($username, $email, 'Thank You for Contacting Westland Mutual Insurance', $body);

        
            $placeholders = [
                '{{name}}'      => htmlspecialchars(admin_name),
                '{{msg}}'  => htmlspecialchars($usermessage),
            ...
            $body = file_get_contents(dirname(__DIR__) . '/partial/contact_detail_email_template.php');
            $body = str_replace(array_keys($placeholders), array_values($placeholders), $body);
            send_mail(admin_name, admin_email, 'New Support Request from Website Contact Form', $body);
            $data["msg"] = "Thank you for contact.";
            $data["status"] = "success";
        } else {
